...
export default function App() {
  ...
  // Safety reset for connecting state — mantém "Conectando..." por até 60s
  // (mesma janela do popup do OAuth do Google) para nunca travar o dispositivo
  useEffect(() => {
    if (!isConnectingGoogle) return;
    const t = setTimeout(() => setIsConnectingGoogle(false), 60000);
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-800 overflow-hidden">
          <div className="bg-gradient-to-br from-[#0F3B2E] to-[#0A2B21] p-8 text-center text-white relative">
            ...
            <div className="relative z-10 space-y-3">
              <img
                ...
                referrerPolicy="no-referrer"
              />
              <h1 className="text-lg font-bold tracking-tight">Dra. Fabrícia Rodrigues • Podologia & Enfermagem</h1>
              <p className="text-[10px] text-[#C8A45A] font-semibold uppercase tracking-widest">Saúde & Bem-Estar</p>
          ...
          <div className="p-6 space-y-4">
            ...
            <button
              onClick={onLogin}
              disabled={isLoggingIn}
              className="w-full flex items-center justify-center gap-2.5 bg-[#0F3B2E] hover:bg-[#1B523E] text-white font-bold py-3 px-4 rounded-xl shadow-sm hover:shadow-lg hover:shadow-[#0F3B2E]/20 transition-all text-xs cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed active:scale-[0.98]"
            >
              {isLoggingIn ? (
                <>
            ...
            <div className="flex items-center justify-center gap-1.5 text-[9px] text-emerald-600 dark:text-emerald-400 font-semibold">
